...
public class MemoryTest {
  ...
  static final List<Integer> sizes =
      ImmutableList.of(4000, 8000, 400000, 800000, 40000000, 80000000);
   ...
   * @throws Exception if a error occured while running the test
   */
    ...
    if (args.length == 0) {
      ...
      for (int size : sizes) {
        for (String benchmark : ArrayListBenchmark.benchmarks.keySet()) {
          ...
          childProcess = new ProcessBuilder().command(javaArgs).inheritIO().start();
