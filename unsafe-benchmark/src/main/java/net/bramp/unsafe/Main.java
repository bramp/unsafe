...
public class Main {
  ...
  public static class UnsafeTester {
    ...
    static final int SIZE = 40000000; // 40000000 uses ~3.1GiB of RAM. TODO work out what I expect
    static final Stopwatch stopwatch = Stopwatch.createUnstarted();

    public void go() {

      final ArrayList<LongPoint> safeList = new ArrayList<LongPoint>(SIZE);
      final UnsafeArrayList<LongPoint> unsafeList =
          new UnsafeArrayList<LongPoint>(LongPoint.class, SIZE);
    ...
    private void recordResult(int trial, String test, String extra) {
      stopwatch.stop();
      results
          .add(new Object[] {test, trial, stopwatch.elapsed(TimeUnit.MILLISECONDS) / 1000d, extra});
      System.gc();
    }
