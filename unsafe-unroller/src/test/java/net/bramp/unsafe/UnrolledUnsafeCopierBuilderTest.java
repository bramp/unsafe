...
public class UnrolledUnsafeCopierBuilderTest {
  ...
  @Test public void testZeroIterations() throws Exception {
    ...
    try {
      UnsafeCopier copier =
          new UnrolledUnsafeCopierBuilder().of(FourLongs.class).length(0).build(unsafe);
  ...
  @Test public void testOneIterations() throws Exception {
    ...
    try {
      UnsafeCopier copier =
          new UnrolledUnsafeCopierBuilder().of(FourLongs.class).length(8).build(unsafe);
  ...
  @Test public void testTwoIterations() throws Exception {
    ...
    try {
      UnsafeCopier copier =
          new UnrolledUnsafeCopierBuilder().of(FourLongs.class).length(16).build(unsafe);
  ...
  @Test public void testFourIterations() throws Exception {
    ...
    long src = copyToMemory(new FourLongs(1, 2, 3, 4));
    try {
      UnsafeCopier copier = new UnrolledUnsafeCopierBuilder().of(FourLongs.class).build(unsafe);
